...
const voucherController = {
	...
	async createVoucher(req, res) {
		try {
			...
			let total = 500000;
			if (req.body.rank == 'silver') total = 2000000;
			else if (req.body.rank == 'gold') total = 5000000;
			else if (req.body.rank == 'platinum') total = 10000000;
	...
	async handleValidVoucher(voucherId, userId) {
		try {
			const check = await db.Voucher_User.findOne({
				where: {
					[Op.and]: [{ ID_Voucher: voucherId }, { ID_User: userId }],
				},
			});
