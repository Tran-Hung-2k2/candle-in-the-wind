    ...
    return (
        <div className={cx('wrapper')}>
            <div className={cx('paginator')}>
                <div className={currentPage === 1 ? cx('paginator-button-disabled') : cx('paginator-button')}>
                ...
                {paginatorButtons &&
                    ...
                    paginatorButtons.map((item, idx) => {
                        if (item === currentPage)
                            ...
                            return (
                                <div key={idx} className={cx('paginator-button')} onClick={() => setCurrentPage(item)}>
                                    {item}
                                </div>
                            );
                    })}
                <div className={currentPage === numberPage ? cx('paginator-button-disabled') : cx('paginator-button')}>
